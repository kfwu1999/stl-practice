 ...
 * \param start: Iterator pointing to the starting node where the heapify
 * process begins.
 */
template <typename _RandomAccessIter, typename _Compare>
void __heapify(_RandomAccessIter first, _Compare comp,
               typename std::iterator_traits<_RandomAccessIter>::difference_type len,
               _RandomAccessIter                                                 start) {
    //
 ...
 *
 * Swaps the value in the position `first` and the value in the position `last -
 * 1`, and makes the subrange `[first, last - 1)` into a heap.
 *
 * \tparam _RandomAccessIter: Type of the iterator used, must support random
 *         access to elements (e.g., iterators of std::vector).
 * \tparam _Compare: Type of the comparison functor that determines the heap
