...
/**
 * \brief Integrates a newly added element into an existing heap, maintaining
 * the heap property.
 *
 * Performing "sift-up" operation to the newly added element in the heap,
 * ensuring the heap property is maintained according to a specified comparison
 * functor. After calling push_heap, the entire range [first, last) will satisfy
 * the heap property.
 *
 ...
 * \param last: Iterator to the last element of the range, which is the newly
 * added element. \param comp: Comparison functor that defines the heap order.
 */
...
void push_heap(_RandomAccessIter first, _RandomAccessIter last, _Compare comp) {
    ...
    //
    difference_type childIdx =
        len - 1;   // last element in the range of the heap (newly added element).
