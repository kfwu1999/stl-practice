 ...
 *
 * Converts the heap [first, last) into a sorted range. After calling this
 * function, the heap property is no longer maintained.
 *
...
void sort_heap(_RandomAccessIter first, _RandomAccessIter last, _Compare comp) {
    ...
    //
    while (last != first) mystl::pop_heap(first, last--, comp);
}
