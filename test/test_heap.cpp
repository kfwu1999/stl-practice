...
protected:
    ...
    void SetUp() override {
        //
        random_array = {74, -42, 48,  -44, 14, 5,  96,  -98, -80, 18,
...
TEST_F(HeapFunctionTests, DuplicateElementIsHeap) {
    ...
    EXPECT_EQ(mystl::is_heap(dup_elems.begin(), dup_elems.end()),
              std::is_heap(dup_elems.begin(), dup_elems.end()));
}

// Test `is_heap` for max heap
TEST_F(HeapFunctionTests, MaxHeapIsHeap) {
    EXPECT_TRUE(mystl::is_heap(max_heap.begin(),
                               max_heap.end()));   // the correctness of is_heap for max heap
    EXPECT_EQ(mystl::is_heap(max_heap.begin(), max_heap.end()),
              std::is_heap(max_heap.begin(),
                           max_heap.end()));   // the result should be same as std::is_heap
}
...
TEST_F(HeapFunctionTests, MaxHeapCreatedByMakeHeap) {
    ...
    EXPECT_TRUE(std::is_heap(random_array.begin(), random_array.end()))
        << "Failed to create a max heap with random array";

    // create max heap by a max heap
    mystl::make_heap(max_heap.begin(), max_heap.end());
    EXPECT_TRUE(std::is_heap(max_heap.begin(), max_heap.end()))
        << "Failed to create a max heap with max heap";

    // create max heap by a min heap
    mystl::make_heap(min_heap.begin(), min_heap.end());
    EXPECT_TRUE(std::is_heap(min_heap.begin(), min_heap.end()))
        << "Failed to create a max heap with min heap";
}
...
TEST_F(HeapFunctionTests, MinHeapCreatedByMakeHeap) {
    ...
    EXPECT_TRUE(std::is_heap(random_array.begin(), random_array.end(), std::greater<>()))
        << "Failed to create a min heap with random array";

    // create min heap by a max heap
    mystl::make_heap(max_heap.begin(), max_heap.end(), std::greater<>());
    EXPECT_TRUE(std::is_heap(max_heap.begin(), max_heap.end(), std::greater<>()))
        << "Failed to create a min heap with max heap";

    // create min heap by a min heap
    mystl::make_heap(min_heap.begin(), min_heap.end(), std::greater<>());
    EXPECT_TRUE(std::is_heap(min_heap.begin(), min_heap.end(), std::greater<>()))
        << "Failed to create a min heap with min heap";
}
